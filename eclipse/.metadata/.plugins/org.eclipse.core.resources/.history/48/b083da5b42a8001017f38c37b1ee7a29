...
//            Render, EntityDrywall, EnumArt, Tessellator, 
//            MathHelper, RenderManager, World, Entity

public class RenderDrywall extends Render
{

    public RenderDrywall()
    {
        rand = new Random();
    }

    public void func_158_a(EntityDrywall entityDrywall, double d, double d1, double d2, 
            float f, float f1)
    {
        rand.setSeed(187L);
        GL11.glPushMatrix();
        ...
        EnumArt enumart = entityDrywall.art;
        float f2 = 0.0625F;
        GL11.glScalef(f2, f2, f2);
        func_159_a(entityDrywall, enumart.sizeX, enumart.sizeY, enumart.offsetX, enumart.offsetY);
        GL11.glDisable(32826 /*GL_RESCALE_NORMAL_EXT*/);
        GL11.glPopMatrix();
    }

    private void func_159_a(EntityDrywall entityDrywall, int i, int j, int k, int l)
    {
        ...
        for(int i1 = 0; i1 < i / 16; i1++)
        {
            for(int j1 = 0; j1 < j / 16; j1++)
            {
                ...
                func_160_a(entityDrywall, (f4 + f5) / 2.0F, (f6 + f7) / 2.0F);
    ...
    private void func_160_a(EntityDrywall entityDrywall, float f, float f1)
    {
        int i = MathHelper.floor_double(entityDrywall.posX);
        int j = MathHelper.floor_double(entityDrywall.posY + (double)(f1 / 16F));
        int k = MathHelper.floor_double(entityDrywall.posZ);
        if(entityDrywall.direction == 0)
        {
            i = MathHelper.floor_double(entityDrywall.posX + (double)(f / 16F));
        }
        if(entityDrywall.direction == 1)
        {
            k = MathHelper.floor_double(entityDrywall.posZ - (double)(f / 16F));
        }
        if(entityDrywall.direction == 2)
        {
            i = MathHelper.floor_double(entityDrywall.posX - (double)(f / 16F));
        }
        if(entityDrywall.direction == 3)
        {
            k = MathHelper.floor_double(entityDrywall.posZ + (double)(f / 16F));
        }
    ...
    public void doRender(Entity entity, double d, double d1, double d2, 
            float f, float f1)
    {
        func_158_a((EntityDrywall)entity, d, d1, d2, f, f1);
    }
